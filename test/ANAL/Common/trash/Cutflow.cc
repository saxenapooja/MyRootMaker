...
Cutflow::Cut::Cut(unsigned int index):
	index(index), pass(false)
{
}

Cutflow::Cutflow(const char* name):
	name_(name)
{
...
void Cutflow::Print() const
{
  ...
  if(i == cuts.size())  cout << "Survives the full cutflow" << std::endl;
}
...
{
  cout<<"cutflows.size() :"<< cutflows.size() << endl;
  cout<<"return :"<< (1 << cutflows.size())<< endl;
