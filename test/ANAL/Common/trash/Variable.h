...
#ifndef _WHANALYSIS_VARIABLE_H_
#define _WHANALYSIS_VARIABLE_H_
...
struct FillInfo
{
  ...
  FillInfo(const RegionDef& regionDef, const FakeRateFunctions& fr, const MvaEvaluator& mvaEvaluator, const IrredMVA& irredMVA,
           const METWeight* metWeight, const MvaTree::Variables& vars, bool mvaCut,
           const Lepton& lepton,       const TauCand& ostau,        const TauCand& sstau);
  ...
  const double met;
  const bool mvaCut; // whether the MVA cut was applied, decides in which selection the event goes. TODO: Should do the same for "cut".
  const double irredMVAValue;

  const double ssMassWeight;
  const double ssFakeMetWeight;
...
public:
  ...
  static const int FAKE_SS_WZ                  = 1 << 2;
  static const int FAKE_SS_ERR                 = 1 << 3;
  static const int FAKE_SS_UNWEIGHTED          = 1 << 4;
  static const int FAKE_12_WZ                  = 1 << 7;
  static const int FAKE_12_ERR                 = 1 << 8;
  static const int FAKE_12_UNWEIGHTED          = 1 << 9;

  static const int MVA_OPTIMIZATION            = 1 << 16;
