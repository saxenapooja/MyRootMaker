...
{
  const int regions = 1; //info.regions;
  const double ssFakeMetWeight = 1.0; // info.ssMassWeight * info.ssFakeMetWeight; //modified on Aug 22
...
void VariableByCategory<HistoType>::fill(const typename Variable<HistoType>::Filler& filler, double weight, const std::vector<const FillInfo*>& info) const
{
  ...
  const double Irred = 1.0; //info[0]->irredMVAValue;
