...
float fZPhi = 0; float fPhi = 0; float fMet = 0; float fNJet = 0; float fWeight = 0;
...
void fitGraph(TTree *iTree,TTree *iTree1, TCanvas *iC,
  ...
  RooRealVar    lRPt  ("pt","Z_{p_{T}}",8,1000);
  RooRealVar    lA1Sig("a1sig","a1sig",0.6,0,1); 
  RooRealVar    lB1Sig("b1sig","b1sig",0. ,-0.5,0.5); 
  ...
  RooRealVar    lA2Sig("a2sig","a2sig",2.5,1.,3.); 
  RooRealVar    lB2Sig("b2sig","b2sig",0.  ,-0.5,0.5); 
  RooRealVar    lC2Sig("c2sig","c2sig",0.  ,-0.1,0.1);  //lC2Sig.setConstant(kTRUE);
  RooRealVar    lD2Sig("d2sig","d2sig",0.  ,-0.1,0.1);  lD2Sig.setConstant(kTRUE);
  ...
  RooRealVar lRXVar("XVar","(U_{1}(Z_{p_{T}})-x_{i})/#sigma_{1} (Z_{p_{T}})",0,-5,5); //lRXVar.setBins(100);
  ...
  for(int i1 = 0; i1 <  iTree->GetEntries(); i1++) {
    ...
    if(iPol1 && fZPt > 50. && lPar == fU1 && lPar  > (48.5-0.95*fZPt) && iType == 0) continue; //Remove Diboson events at high pT
    //if(fU1 > -4) continue;
    if(fZPt > fZPtMax) continue;
    if(fMet > fMetMax) continue;
    if(fNJetSelect<2  && fNJet != fNJetSelect && fNJetSelect != -1) continue;
  ...
  double lZPtMin = 0; if(fNJetSelect != -1 && !iRMS) lZPtMin = 20;
  if(iRMS) lZPtMin  = 10;
  TFitResultPtr  lFitPtr = pGraphA->Fit(lFit,"SR","",lZPtMin,fZPtMax); //"EXO"
  ...
  RooFitResult *pFR = lGAdd.fitTo(lResidVals2D[0],Save(kTRUE),ConditionalObservables(lRPt),NumCPU(2),Minos()); //Double Gaussian fit
  /*
  //Plot it all
  lRXVar.setBins(100);
...
void runRecoilFit_v6() { 
  ...
  fDataFile = new TFile("../Ntupler/Data_v2/r12-dimu_2012.root");
  fDataTree = (TTree*) fDataFile->FindObjectAny("Flat"); fData = true; fId = 0;
  fId =  0; fNJetSelect =  0; fMetMax = 4000; fZPtMax =  70;  fitRecoil(fDataTree,"recoilfits/recoilfit_datamm53X_2012_0jetp.root");
  fId =  0; fNJetSelect =  1; fMetMax = 4000; fZPtMax = 200;  fitRecoil(fDataTree,"recoilfits/recoilfit_datamm53X_2012_1jetp.root");
  fId =  0; fNJetSelect =  2; fMetMax = 4000; fZPtMax = 500;  fitRecoil(fDataTree,"recoilfits/recoilfit_datamm53X_2012_2jetp.root");
  delete fDataFile;
  //return;
  fDataFile = new TFile("../Ntupler/Data_v2/s12-zjets_2012.root");
  fDataTree = (TTree*) fDataFile->FindObjectAny("Flat"); fData = false; fId = 0;
  fId =  0; fNJetSelect =  0; fMetMax = 4000; fZPtMax =  70;  fitRecoil(fDataTree,"recoilfits/recoilfit_zmm53X_2012_0jetp.root");
  fId =  0; fNJetSelect =  1; fMetMax = 4000; fZPtMax = 200;  fitRecoil(fDataTree,"recoilfits/recoilfit_zmm53X_2012_1jetp.root");
  fId =  0; fNJetSelect =  2; fMetMax = 4000; fZPtMax = 500;  fitRecoil(fDataTree,"recoilfits/recoilfit_zmm53X_2012_2jetp.root");
  delete fDataFile;
  return;
