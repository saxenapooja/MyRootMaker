...
RootMaker::RootMaker(const edm::ParameterSet& iConfig) :  
  ...
  cAK5PFPt4RC(iConfig.getUntrackedParameter<double>("RecAK5PFPtMin4RC", 30.)),
  cAK5PFEta4RC(iConfig.getUntrackedParameter<double>("RecAK5PFEtaMin4RC", 2.4)),
...
void RootMaker::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  if(crectautaupairs)
    {
      haveDiTaus =  AddTauTauPairs(iEvent);
    }
	...
	for (unsigned indxTau1 =0; indxTau1 < tauIndexSelection.size() ; indxTau1++) 
	  {
		...
		for(size_t i = 0; i < jetColl.size(); i++) {
		  LorentzVector JetP4(jetColl[i].px(), jetColl[i].py(), jetColl[i].pz(), jetColl[i].energy());
		  //cout<<"(dR1,dR2) :"<< ROOT::Math::VectorUtil::DeltaR(JetP4, Leg1p4)  <<", "<< ROOT::Math::VectorUtil::DeltaR(JetP4, Leg2p4) << endl;
		...
		if(GenParticles.isValid())
		  {
		    ...
		    for(unsigned i = 0 ; i < GenParticles->size() ; i++) {
		      ...
		      if(abs(boson->pdgId()) == 23 ||  abs(boson->pdgId()) == 24 || abs(boson->pdgId()) ==25 || abs(boson->pdgId()) ==35 || abs(boson->pdgId()) == 36 ) 
			{
			  recCorrection = RecoilCorrectedMET(MET, Leg1p4, Leg1p4, boson, sampleName, njets4RC);
			  diTau_RecCorr_Px[ditau_Index] = recCorrection.Px();
			  diTau_RecCorr_Py[ditau_Index] = recCorrection.Py();
			  diTau_RecCorr_Pz[ditau_Index] = recCorrection.Pz();
			  diTau_RecCorr_E[ditau_Index]  = recCorrection.E();
			}
		    }
		  }
		ditau_Index++; 
...
bool RootMaker::AddAK5JPTJets(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
	...
	iEvent.getByLabel(edm::InputTag("patJets"), ak5jptJets);
...
double RootMaker::ComputeDiTauMass(LorentzVector leg1, LorentzVector leg2, LorentzVector met, TMatrixD cov)
{
...
TLorentzVector RootMaker::RecoilCorrectedMET(LorentzVector pfMet_, LorentzVector Leg1p4_, LorentzVector Leg2p4_, const reco::GenParticle *boson_, string sampleName_, int nJets_)
{
